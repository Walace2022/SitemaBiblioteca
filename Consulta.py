...
def mostrar_alunos():
    ...
    exibir_dados(dados_alunos, "Lista de Alunos")
...
def mostrar_professores():
    ...
    exibir_dados(dados_professores, "Lista de Professores")
...
def main():
    ...
    JanelaConsulta.geometry("430x600")

   
    frame = Frame(JanelaConsulta)
    ...
    frame_menu = Frame(JanelaConsulta)
    frame_menu.pack(pady=10)  

    
    botao_alunos = ttk.Button(frame_menu, text="Alunos", command=mostrar_alunos)
    botao_alunos.pack(side="left", padx=10)

    botao_professores = ttk.Button(frame_menu, text="Professores", command=mostrar_professores)
    ...
    canvas = Canvas(JanelaConsulta, bg="white")
    canvas.pack(side="right", fill="both", expand=True)

    scrollbar = ttk.Scrollbar(JanelaConsulta, orient="vertical", command=canvas.yview)
